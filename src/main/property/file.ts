...
export default class Property extends DynamicProcessor() {
	...
	async _process(request) {
		...
		const exists = (await fs.promises.exists(this.#file)) && (await fs.promises.stat(this.#file)).isFile();
		if (this._request !== request) return;
		if (!exists) return done({ errors: `Configuration file "${this.#file}" not found` });

		const content = await fs.promises.readFile(this.#file, { encoding: 'UTF8' });
		if (this._request !== request) return;
